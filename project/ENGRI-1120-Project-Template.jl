...
html"""
<p style="font-size:20px;">Roy and the Boys: Sasha Logunov, Roarke Creedon, Roy Sibaja, and Daniel Korzukhin</br>
...
md"""
### 1. Introduction
For our project, we were tasked with creating a sustainable production process for the synthesis of 1-2- propanediol dinitrate - also known as PGDN and Propylene glycol dinitrate - from the base sugar Maltose. The client contracted Olin Engineering, specifically our team Roy and the Boys, to design this process to produce a >95% pure 1-2- propanediol dinitrate at a rate of at least 1 gram/hour. Maltose is a disaccharide containing 6 carbons, formed from the combination of 2 glucose molecules. Our product, 1-2- propanediol dinitrate is an organic molecule which is flammable and explosive. The main current use of this product is as a propellant in Otto Fuel II, which is used as propellant in some torpedoes. It’s also used as a medicine and has been used as an explosive for centuries. 

For the design process, we have access to:
Well-mixed reactors, with two input streams and a single output stream. These cost $ 100.00 each. 
Magical separators which intend to separate unreacted starting materials from our desired product. These cost $ 20.00. 
Stream splitters which are used to separate products of each reaction from one another. These are free of charge. 
Syringe pumps which feed the reactor units. These cost $ 1000.00 each. 

With all of this being said, Varnerlab has complete faith in our design team and has invested more than enough money into this project. Budget will be no issue in our design process, but we are aiming for the cash* and prizes, and we hope to create a design process as efficiently as possible. 
To make this project feasible we had to make several assumptions: 
all reactions isothermic, all chips at steady state, all reactions ideal and proceed to completion, no cost for waste disposal or equipment upkeep, and constant chemical prices 
"""
...
begin
	...
	compounds_that_we_need_to_supply_feed_1 = [
		"nadh", "atp","potassium nitrate","maltose"
	]
...
# ╔═╡ e925da7d-3e0c-4f0e-8172-d303c5cb9dea
number_of_chips = 11
...
# ╔═╡ f9bf7055-d319-4adf-8e2c-82ebc4b775eb
md"""
##### Net Present Value (NPV)
"""

# ╔═╡ c4c767de-2840-4221-af63-7447daeb8f3f
# lifetime = 3 future years (period of time to payback the investment)
T = 5

# ╔═╡ ab646109-c1c2-4818-93df-7de416c4da1b
...
# ╔═╡ f503660f-baf0-48b5-a16a-afab37e01d04
# Setup the cash flow array -
# we pay in year 1: $230K (out, -ve)
# we save in year(s) 2,3 and 4: 90K (in,+ve)
CF_array = [-2.22, 24.2652,24.2652,24.2652,24.2652,24.2652]

# ╔═╡ 21f6d971-9082-423f-81ce-d58db4c3bedb
...
# ╔═╡ 69340c27-a3e8-4372-bd55-931c4651b088
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ 450f9af2-607a-488e-9c44-abea4e69e7a0
# what is the NPV?
NPV = sum(PV)
...
In our project we found that the only reactants we need in order to maximize the economic efficiency of our system are potassium nitrate, glycerol, and maltose. Based on the efficiency of the reactor chip it was determined the maximum flow rate of our product could be 2.466 mmol/hr, so this was our target flow rate for the PDGN. In order to minimize the number of separators that need to be used for our reaction we had to find the smallest amount of potassium nitrate, glycerol, and maltose that needs to be put in to achieve the desired output rate (2.466 mmol/hr). This ensures that none of the reactants would come out of the output stream. Based on the flow rate table we determined that we needed to feed 4.932 mmol/hr of potassium nitrate, 1.233 mmol/hr of glycerol, and 1.233 mmol/hr of maltose. This allowed us to make 0.409575 g/hr of unfiltered PDGN from one reactor chip. After this the PDGN needs to be separated from the other unwanted waste products, as we are only able to sell a 95% pure product. Since our separators are only 75% efficient, mutliple separators that are in series need to be used in order to obtain the desired purity. According to our data we needed 5 separators in order to get to the 95% purity. In fact, after 5 separations we obtained a 98.1519% pure product. However, we determined that we would be able to sell the PGDN for a higher price if we could get it to at least 99% purity. Therefore, we decided to add an additional 6th separator to help us get a 99.38% pure product. This caused us to only obtain 0.0978042 g/hr of PDGN from 1 reactor, but 1g/hr is the necessary production rate. This means we need more reactor chips in order to reach the desired flow rate. Specifically we need 10 more chips to get the minimum 1g/hr. With the 11 reactor chips we are able to obtain 1.07585g/hr of PDGN. 
...
md"""
### 4. Conclusions
Looking at the financial aspect of our project we initially had to determine how much each of the products would cost us. From Sigma Aldrich, we can buy 25kg of the potassium nitrate for $ 1590.00. This means that it will cost us $ 0.032/hr to feed potassium nitrate into the system. We are also buying Maltose from Sigma Aldrich at a cost of $ 152.0 per 1kg. This would mean that the price of maltose per hour would be $ 0.068/hr. Lastly, we can buy Glycerol from Sigma Aldrich at a rate of $ 46.60 per 500mL, which translates to $ 0.0094/hr. That means we are spending $ 0.1094/hr to make our reaction occur. Looking at the products that we can sell, we can only sell the 1,2-propanediol dinitrate due to the fact that it is the only product that has a minimum of 95% purity. Since there is no price listed for PGDN on Sigma Aldrich, the price of PGDN will be calculated by finding how much potassium nitrate we can sell combined with the price of 1,2-propanediol. The price of the potassium nitrate that we are selling 100g for is $ 61.80. PGDN has a “dinitrate”, which means we need to account for two Potassium Nitrate groups for every one of 1,2- propanediol. This means that we would make $ 1.31/hr for the potassium nitrate. We are selling 25.9g of 1,2- propanediol for $ 38.50. This means that we would make $ 1.57/hr for the 1,2-propanediol. This means that we would be able to make $ 2.88/hr from our products. This further leads us to being able to find our overall net profit of the system, which would be $ 2.77/hr. The price of the reactor chips and the stream separators also have to be taken into account. The price for each reactor chip is $ 100.00, so since we are using 11 reactor chips the amount of money we are spending on them is $ 1100.00. We also used 6 separators in our reaction and each one costs $ 20.00, so we are spending $ 120.00 on separators. The syringe pump that inputs the reactants into the system at a continuous rate costs $ 1000.00. The total equipment cost is $ 2220.00 This means it would take 801.44 hrs for us to break even from the setup cost for our system. If we keep the system running constantly, we will profit $ 22045.20 in the first year and $ 24265.20 in each successive year. 

Using the NPV analysis code provided for us, we analyzed the financial performance of our system against an alternative investment of our starting funds yielding a 1-10% return. Judging from the positive NPV values calculated, our system outperformed a 10% investment return over a 1, 5 and 10 year lifetime. 
...
md"""
### References
Milliporesigma. MilliporeSigma | United States. (n.d.). Retrieved December 8, 2021, from https://www.sigmaaldrich.com/US/en?gclid=Cj0KCQiAzMGNBhCyARIsANpUkzNnvopiR7j5s3FrKf2dJjIXOfd398n4LT9QMFFTzytZ5CG5lFcXcCYaAhfgEALw_wcB. 

U.S. National Library of Medicine. (n.d.). Propylene glycol dinitrate. National Center for Biotechnology Information. PubChem Compound Database. Retrieved December 9, 2021, from https://pubchem.ncbi.nlm.nih.gov/compound/22933#section=Use-and-Manufacturing. 
"""
